...
the deterministic load order from `getObjects`. The `write` output is the proof
artefact, so its bytes must match `FORMAT.md` -- including the trailing space after
each vertex line, which `FORMAT.md` omits but the C++ emits.
...
blocked by a reducible configuration (A.8.1). -/
...
/-- Load every `.combined_rule` file in `combinedRuledir`. -/
...
/-- Enumerate all combined rules reachable from the given rules (A.8.2).

The rounds are sequential, but within a round each `addRuleToCombination` is
independent and read-only, so the expansion runs with the order-preserving
`parMap`, computing the identical combined-rule list by construction. -/
