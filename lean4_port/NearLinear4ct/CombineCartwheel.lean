...
order-preserving `parFlatMap` (see Cartwheel.lean). The combinators retain the
fine-grained load balancing needed for per-cartwheel cost skew. A failing proof
obligation throws in its worker; `parForEach` re-raises the first failure in
input order, so the process exits non-zero.
