...
For each internal vertex $v_i$ ($i=R+1, \ldots, N$), the file lists the vertex index $i$, its degree $\delta(v_i)$, and the clockwise rotation of indices of its neighbours $a_i^1, \ldots, a_i^{\delta(v_i)}$.
...
For each vertex $v_i$ ($i=1,\ldots,N$), the file lists the vertex index $i$, its degree-range $\delta^-(v_i), \delta^+(v_i)$, and the clockwise rotation of indices of its neighbours $a_i^1, \ldots, a_i^{d(v_i)}$.
...
The vertex set is $V=\{v_1, \ldots, v_N\}$.
$c$ denotes the index of the centre of a cartwheel.
